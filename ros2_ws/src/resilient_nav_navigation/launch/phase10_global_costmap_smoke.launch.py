"""Compose the healthy Task 1 localization baseline with a Global Costmap."""
...
def generate_launch_description():
    """Start only the Global Costmap; navigation servers remain excluded."""
    ...
    lifecycle_manager = Node(
        ...
        parameters=[{
            'autostart': True,
            # The standalone upstream Costmap executable participates in ROS
            # lifecycle transitions but does not establish Nav2's managed-node
            # bond.  Keep lifecycle transition control while disabling only
            # that inapplicable bond watchdog.
            'bond_timeout': 0.0,
    ...
    rviz = Node(
        package='rviz2',
        executable='rviz2',
        name='phase10_global_costmap_rviz',
        arguments=['-d', str(navigation_share / 'rviz' / 'phase10_global_costmap.rviz')],
        output='screen',
    ...
    return LaunchDescription([
        ...
        # The child forces its own Phase 4 RViz off.  Scope that override so it
        # cannot alter this wrapper's outer use_rviz configuration.
