"""Run the fixed crossing BRNE demo on the frozen Phase 10 planner-only base."""
...
def generate_launch_description():
    """Compose one manual BRNE owner; no RPP Controller or BT is launched."""
    ...
    spawn_pedestrian = Node(
        ...
        parameters=[{
            ...
            # Phase 10's robot starts at world (-3.5, -3.5) and odom (0, 0).
            # The pedestrian therefore starts at odom (0.5, -1.0).
            'x': -3.0,
            'y': -4.5,
            'z': 0.60,
    ...
    rviz = Node(
        package='rviz2',
        executable='rviz2',
        name='brne_closed_loop_demo_rviz',
        output='screen',
        arguments=['-d', str(brne_share / 'rviz' / 'brne_closed_loop_demo.rviz')],
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            default_value='false',
            description=(
                'Explicitly grant BRNE sole /cmd_vel authority. False creates '
                'no formal velocity publisher.'
            ),
        ),
        ...
        Node(
            ...
            name='brne_pedestrian_odometry_adapter',
            output='screen',
            parameters=[{'use_sim_time': True}],
        ),
        Node(
        ...
        Node(
            ...
            name='brne_crossing_pedestrian_driver',
            output='screen',
            parameters=[{'use_sim_time': True}],
        ),
        Node(
